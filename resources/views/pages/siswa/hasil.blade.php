...
<div class="hasil-wrapper">
    ...
    <div class="hasil-container">
        ...
        @php
            ...
            $namaField   = $siswa->nama_lengkap ?? $siswa->nama ?? $siswa->name ?? Auth::user()->name ?? 'Siswa';
            $jurusanIkon = ['Alat Berat'=>'🚜','Otomotif'=>'🚗','Motor'=>'🏍️','Pemesinan'=>'🔧','Mekatronika'=>'🤖','Konstruksi'=>'🏗️','Bangunan'=>'🏗️','Listrik'=>'⚡','Pembangkit'=>'🔋','Audio'=>'📺','Komputer'=>'💻','Desain'=>'🎨'];
            $ikon = '🏆';
            foreach ($jurusanIkon as $kata => $ico) {
                if (str_contains($rekomendasi->jurusan->nama_jurusan ?? '', $kata)) { $ikon = $ico; break; }
            }
        @endphp
        ...
        <div class="info-card">
            ...
            <div class="info-grid">
                <div class="info-item"><div class="info-item-label">Tanggal Tes</div><div class="info-item-value">{{ $tesTerakhir->created_at->format('d M Y') }}</div></div>
                <div class="info-item"><div class="info-item-label">Nilai Akademik</div><div class="info-item-value">{{ number_format($tesTerakhir->nilai_akademik,1) }}</div></div>
                <div class="info-item"><div class="info-item-label">Skor Minat</div><div class="info-item-value">{{ $tesTerakhir->skor_minat_bakat }}%</div></div>
                <div class="info-item"><div class="info-item-label">Tinggi / Berat</div><div class="info-item-value">{{ $tesTerakhir->tinggi_badan }}cm / {{ $tesTerakhir->berat_badan }}kg</div></div>
            </div>
        </div>
        ...
        <div class="ranking-card">
            ...
            <div class="rank-item {{ $rankClass }} {{ (!$rankClass && $isPilihan) ? 'pilihan' : '' }}">
                ...
                <div class="rank-info">
                    ...
                    <div class="rank-bar-wrap"><div class="rank-bar-fill" style="width:0%" data-target="{{ $barWidth }}%"></div></div>
                </div>
        ...
        <div class="saw-card">
            ...
            <div class="table-wrap">
                <table class="saw-table">
                    <thead><tr><th>Rank</th><th>Jurusan</th><th>Nilai Preferensi (V)</th><th>Skor (%)</th></tr></thead>
                    <tbody>
                        @foreach($hasilList as $i => $hasil)
                        <tr class="{{ $i===0 ? 'best' : '' }}">
